    ...
    public partial class frmBuscaEndereco : Form
    {
        ...
        private void btnPesquisar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                txtCompleto.Text = (txtEndereco.Text) + (" ") + (mkdNum.Text) + (" , ") + (txtComplemento.Text) + (" , ") + (mkdCEP.Text) + (" , ") + (txtBairro.Text) + (" , ") + (txtCidade.Text) + (" , ") + (txtUF.Text);
...

            {
                ...
                MessageBox.Show("CEP Inválido", "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
            }
        ...
        private void Copiar_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("O Endereço foi copiado", "Mensagem do Sistema", MessageBoxButtons.OKCancel, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1) == DialogResult.OK)
            {
                txtCompleto.Text = (txtEndereco.Text) + (" , ") + (mkdNum.Text) + (txtComplemento.Text) + (" , ") + (mkdCEP.Text) + (" , ") + (txtBairro.Text) + (" , ") + (txtCidade.Text) + (" , ") + (txtUF.Text);
                textBox1.Text = txtCompleto.Text;
                txtCompleto.SelectAll();
